...
window.hexFlexComparisonHexViewer = {
    ...
    init: function (elementId, dotNetRef) {
        ...
        var state = {
            ...
            dotNetRef: dotNetRef,
            rafPending: false,
            // Virtual rows: array of { isGap, address, segmentIndex, gapStartAddr, gapEndAddr, skippedBytes }
            virtualRows: null,
            totalRowCount: 0,
            // Per-row diff data: for data rows, 16 bytes of { diffType, byteB, byteA }
            // diffTypes: Uint8Array (0=unchanged, 1=modified, 2=added, 3=removed)
            // bytesB: Uint8Array (byte values from file B)
            // bytesA: Uint8Array (byte values from file A)
            // validity: Uint8Array (1=has data, 0=no data)
            diffTypes: null,
            bytesB: null,
            bytesA: null,
            validity: null,
            dataRowIndices: null, // maps data-row sequential index to virtualRows index
            highlightAddr: -1,
    ...
    // Called once from C# after comparison is computed.
    // virtualRowsJson: JSON array of virtual rows
    // diffTypes, bytesB, bytesA, validity: flat Uint8Arrays for all data rows (16 bytes per row)
    setData: function (elementId, virtualRowsJson, diffTypes, bytesB, bytesA, validity) {
        ...
        // Build data row index map (sequential data row index -> position in arrays)
        var dataRowIndex = 0;
...
// CSS class for diff text styling
    ...
    for (var row = firstRow; row <= lastRow; row++) {
        ...
        if (vr.isGap) {
            ...
            for (var j = 0; j < 16; j++) {
                ...
                if (hasData) {
                    var byteVal = (dt === DIFF_REMOVED) ? state.bytesA[idx] : state.bytesB[idx];
                    // Use byteB if available, else byteA
                    if (state.validity[idx] && state.bytesB[idx] !== undefined) {
                        // For display: show B value for modified/added/unchanged, A for removed
                        if (dt === DIFF_REMOVED) {
            ...
            for (var j = 0; j < 16; j++) {
                ...
                if (hasData) {
                    var byteVal;
                    if (dt === DIFF_MODIFIED) {
                        // For modified, show file B ASCII
                        byteVal = state.bytesB[idx];
