...
**Blazor status:** Fixed — JS `goToAddress` clears previous timer with `clearTimeout(state._hlTimer)` before setting new one.
...
**Blazor status:** Fixed — Blazor version uses inline SVG arrows (up/down) with correct semantic meaning.
...
**Blazor status:** N/A — Blazor version has no GenAI dependency.
...
**Blazor status:** Fixed — Blazor uses CSS media queries for ASCII visibility, and shared constants in `Services/Constants.cs`.
...
**Blazor status:** Fixed — Blazor `<ErrorBoundary>` wraps `@Body` in MainLayout with custom error UI and recovery button.
...
**Blazor status:** Fixed — C# `HexParser.cs` handles record type 0x02 (implemented in Phase 1).
